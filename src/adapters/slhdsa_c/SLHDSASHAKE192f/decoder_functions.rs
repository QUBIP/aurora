...
impl Decoder for DER2SubjectPublicKeyInfo {
    const PROPERTY_DEFINITION: &'static CStr =
        c"x.author=QUBIP,qubip.adapter=slhdsa_c,input=der,structure=SubjectPublicKeyInfo";
...
impl Decoder for DER2PrivateKeyInfo {
    const PROPERTY_DEFINITION: &'static CStr =
        c"x.author=QUBIP,qubip.adapter=slhdsa_c,input=der,structure=PrivateKeyInfo";
